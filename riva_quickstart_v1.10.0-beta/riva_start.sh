...
if [ $(docker ps -q -f "name=^/$riva_daemon_speech$" | wc -l) -eq 0 ]; then
    ...
    if [[ $riva_target_arch == "arm64" ]]; then
        docker_run_args="-p 8000:8000 -p 8001:8001 -p 8002:8002 --device /dev/bus/usb --device /dev/snd $image_speech_api riva_server"
    else
        docker_run_args="-p 8000 -p 8001 -p 8002 $image_speech_api start-riva --riva-uri=0.0.0.0:$riva_speech_api_port --asr_service=$service_enabled_asr --tts_service=$service_enabled_tts --nlp_service=$service_enabled_nlp &> /dev/null"
    fi
    docker run -d \
        --init --rm \
        --gpus '"'$gpus_to_use'"' \
