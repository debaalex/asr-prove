...
if [[ $riva_target_arch == "arm64" ]]; then
    ...
    client_cmd="$update_manifest_cmd; /opt/examples/c++/riva_streaming_asr_client  --chunk_duration_ms=1600  --model_name=citrinet-256-en-US-streaming  --interim_results=false  --word_time_offsets=false  --riva_uri=localhost:50051  --automatic_punctuation=false  --audio_file=/data/test_wav/manifest.tmp.json  --output_filename=/data/test_wav/output.json;  python3 /opt/utils/calc_wer.py -test /data/test_wav/output.json -ref /data/test_wav/manifest.tmp.json"
